...
public class BookmarkTable {
  ...
  public final boolean insertBookmark(String new_bookID, String new_date,
                                      String new_name, String new_cfi) {
  ...
  public static final ArrayList<HashMap> getBookmarksForID(String id,
                                                           Context context) {
    ...
    Cursor c =
        Bookmarkdatabase.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " +
                                      bookID + " = \"" + id + "\"",
                                  null);
    while (c.moveToNext()) {
  ...
  public static final boolean deleteBookmark(String arg_date, String arg_name,
                                             Context context) {
    if (Bookmarkdatabase == null) {
      FolioDatabaseHelper dbHelper = new FolioDatabaseHelper(context);
      Bookmarkdatabase = dbHelper.getWritableDatabase();
    }
    String query = "SELECT " + ID + " FROM " + TABLE_NAME + " WHERE " + date +
                   " = \"" + arg_date + "\""
                   + "AND " + name + " = \"" + arg_name + "\"";
